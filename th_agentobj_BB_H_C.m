classdef th_agentobj_BB_H_C
    ...
    properties
        ...
        la;                 % linear convex combination parameter lambda
        ...
        Q_t_BB_R_1;         % action valence (component: BB-R-1)
        Q_t_BB_I;           % action valence (component: BB-I)
        p_t;                % action probability state (hybrid)
        p_t_BB_R_1;         % action probability (component: BB-R-1)
    ...
    % methods of agent variant BB-H-C
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function agentobj = th_agentobj_BB_H_C(agent_init)

        % This function creates an agent object of class th_agentobj_BB_H_C 
        ...
        % computed with the BB-R-1 scheme). This hybrid agent uses a 
        % throughout constant parameter that controls the linear 
        % combination of the two action valences.
        %
        % Inputs
        ...
        V_a_s_BB_R_1        = NaN(1,length(agentobj.A));                    % BB-R-1 state-action valence
        n_V_a_s_BB_R_1      = NaN(1,length(agentobj.A));                    % BB-R-1 normalized state-action valence
        ...
        agentobj.Q_t_BB_R_1 = NaN(1,length(agentobj.A));                    % BB-R-1 action valence (normalized)
        ...
        agentobj.p_t_BB_R_1 = NaN(1,length(agentobj.A));                    % BB-R-1 action probability
        ...
        % ************* rewarding valence: the BB-R-1 scheme **************
        
        % maximum-a-posteriori node
        [~,map_n] = max(curr_bf);                                           
        ...
        for i = 1:length(A_s)
            ...
            % target distances
            V_a_s_BB_R_1(A_s(i)) = agentobj.L1(s_t_t,map_n);

        end
        
        % normalize
        n_V_a_s_BB_R_1 = V_a_s_BB_R_1/nansum(V_a_s_BB_R_1);
        ...
        for a = 1:length(A_s)
           ...
           agentobj.Q_t(A_s(a)) = agentobj.la*-n_V_a_s_BB_R_1(A_s(a)) + (1-agentobj.la)*n_V_a_s_BB_I(A_s(a));  
           
        end
        
        % set normalized action valences
        agentobj.Q_t_BB_I   = n_V_a_s_BB_I;
        agentobj.Q_t_BB_R_1 = -n_V_a_s_BB_R_1;
        ...
        agentobj.p_t_BB_R_1(A_s) = th_softmax(-n_V_a_s_BB_R_1(A_s),agentobj.be); % BB-R-1
