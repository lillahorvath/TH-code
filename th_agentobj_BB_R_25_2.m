classdef th_agentobj_BB_R_25_2

    % This class definition file specifies the properties and methods of an
    % object that implements agent variant BB-R-25-2.
    %
    % Copyright (C) Dirk Ostwald, Lilla Horvath
    % ---------------------------------------------------------------------
    
    % properties of agent variant BB-R-25-2
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    properties
    ...
    % methods of agents variant BB-R-25-2
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function agentobj = th_agentobj_BB_R_25_2(agent_init)

        % This function creates an agent object of class 
        % th_agentobj_BB_R_25_2 based on the agent initialization input 
        % structure
        %
        % Inputs
        ...
        % current belief state. It uses a sequential one-step look ahead 
        % procedure: first it evaluates the action valences based on the 
        % complete belief state distribution, such as BB-R-25. If there
        % appear to be more than one best action using this scheme the
        % agent switches to the BB-R-2 strategy and reevaluates the action
        % valences. ((1) it identifies the two maximum-a-posteriori target 
        % locations, then (2) identifies the target which is closer to the 
        % agent's current position and (3) uses this target node to 
        % evaluate the action valences)
        %
        % Inputs
        ...
        % --------------- standard case: the BB-R-25 scheme ---------------
        ...
        V_a_s = round(V_a_s, 2);                                            % round values to 2 decimals
        aoi = find(V_a_s == min(V_a_s));                                    % array of interest: index minimal valences (=best actions)
        
        % -------- ambiguous decision situation: switch to BB-R-2 ---------
        
        % if there is more than one best action
        if numel(aoi) > 1
            
            V_a_s        = NaN(1,length(agentobj.A));                       % reinitialize state-action valence
            aoi          = [];
            aoi          = round(agentobj.B(:,end), 10);                    % extract array of interest: current belief state
            [~,sort_ind] = sort(aoi(~isnan(aoi)));                          % sort and list the index values
            
            % if two remaining targets in attempt: idenify the two maximum-
            % a-posteriori nodes and get the one that is closer 
            if agentobj.s_t(2) == 0 
            ...
            % if one remaining target in attempt: get the maximum a-
            % posterriori-node    
            else
                map_n = sort_ind(end);
            end
            ...
            for j = 1:length(A_s)
                ...
                % one-step look ahead
                s_t_t = agentobj.s_t(1) + agentobj.A(A_s(j));

                % target distances
                V_a_s(A_s(j)) = agentobj.L1(s_t_t,map_n);
