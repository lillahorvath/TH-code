classdef th_agentobj_BB_R_2
    ...
    % methods of agents variant BB-R-2
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function agentobj = th_agentobj_BB_R_2(agent_init)

        % This function creates an agent object of class th_agentobj_BB_R_2 
        ...
        % (1) it identifies the two maximum-a-posteriori target locations, 
        % (2) identifies the target which is closer to the agent's current 
        % position and (3) uses this target node to evaluate the action 
        % valences
        %
        % Inputs
        ...
        % action valence evaluation: compute the l1 distance between the 
        % action-outcome node and the closer maximum-a-posteriori belief 
        % -----------------------------------------------------------------
        curr_bs      = round(agentobj.B(:,end), 10);                        % get the agent's current belief state 
        [~,sort_ind] = sort(curr_bs(~isnan(curr_bs)));                      % sort and list the index values
        ...
        for i = 1:length(A_s)
            ...
            % target distances
            V_a_s(A_s(i)) = agentobj.L1(s_t_t,map_n);
